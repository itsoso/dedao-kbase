...
var staticBookKnowledgePrompts = []BookKnowledgePrompt{
	...
	staticPrompt("project-health-kb", "项目导入", "Health KB 条目", "导入 health-llm-driven", "请把本书转换成 health-llm-driven 可导入的健康知识条目：实体、claim、证据、适用人群、风险边界、review_status。每条都必须引用 claim_id 或 chunk_id。", "jsonl"),
	staticPrompt("project-quant-rules", "项目导入", "量化规则卡", "导入 macd-analysis-claude", "请把本书转换成 macd-analysis-claude 的 paper-only 量化规则卡：市场假设、入场条件、退出条件、风控条件、回测需求、禁止实盘说明。每张规则卡引用 claim_id 或 chunk_id。", "json"),
...
func dynamicBookKnowledgePrompts(pkg *BookKnowledgePackage) []BookKnowledgePrompt {
	...
	return []BookKnowledgePrompt{
		...
		dynamicPrompt("dynamic-project-fit", "本书专属", "项目适配建议", "判断适合导入哪些项目",
			fmt.Sprintf("请判断《%s》更适合导入哪些项目或知识库：health-llm-driven、macd-analysis-claude、NotebookLM、通用 wiki。请按适配度排序，并引用 claim_id 或 chunk_id 说明原因。", book.Title), "markdown"),
