...
type AgentEvaluationCase struct {
	...
	ObservedArguments map[string]string `json:"observed_arguments,omitempty"`
}
...
func EvaluateAgentPackageDeterministically(pkg AgentPackage, suite AgentEvaluationSuite, now time.Time) (AgentEvaluationReport, error) {
	...
	for index, evalCase := range suite.Cases {
		...
		if deterministicAgentEvaluationCasePasses(evalCase) {
...
func deterministicAgentEvaluationCasePasses(evalCase AgentEvaluationCase) bool {
	switch evalCase.Metric {
	case "retrieval", "citations", "faithfulness":
		if len(evalCase.ExpectedIDs) == 0 {
			return false
		}
		observed := stringSet(evalCase.ObservedIDs)
		...
		return true
	case "abstention", "tool_choice":
		return strings.TrimSpace(evalCase.ExpectedValue) != "" &&
			strings.TrimSpace(evalCase.ExpectedValue) == strings.TrimSpace(evalCase.ObservedValue)
	case "tool_arguments":
		return len(evalCase.ExpectedArguments) > 0 && reflect.DeepEqual(evalCase.ExpectedArguments, evalCase.ObservedArguments)
	default:
		return false
	}
}
...
func (s *BookKnowledgeStore) SaveAgentPackageEvaluation(report AgentEvaluationReport) error {
