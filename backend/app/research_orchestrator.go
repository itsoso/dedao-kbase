...
func researchRoleSystemPrompt(role ResearchModelRole) (string, error) {
	...
	case ResearchRoleExtractor:
		schema = `Schema: {"decision_summary":"string","facts":[Fact],"claims":[Claim],"measurements":[Measurement],"cases":[Case]}. Fact JSON shape: {"fact_id":"string","kind":"string","summary":"string","status":"string","occurred_at":"RFC3339 or empty string","evidence_ids":["ID"],"confidence":0.8,"review_state":"pending"}. Claim JSON shape: {"claim_id":"string","kind":"string","topic":"string","value":"string","timing":"string","amount":"string","applies_to":"string","evidence_ids":["ID"],"confidence":0.8,"review_state":"pending"}. Measurement JSON shape: {"measurement_id":"string","name":"string","value":0.0,"occurred_at":"RFC3339","evidence_ids":["ID"],"confidence":0.8,"review_state":"pending"}. Case JSON shape: {"case_id":"string","role":"historical|current","age":0,"stage_day":0,"symptoms":["string"],"measurements":{"metric_name":0.0},"recovery_status":"string","evidence_ids":["ID"]}. Every emitted item needs at least one exact runtime-supplied evidence ID; omit an item when no exact supplied evidence ID supports it. age, stage_day, and every measurement value must be JSON numbers, never quoted strings; use 0 or {} when the evidence does not supply them. Every review_state must be "review_state"("pending"|"verified"|"rejected"); use "pending" for newly extracted items because verification happens later. Use [] for categories with no grounded items; confidence is (0,1].`
	case ResearchRoleSynthesizer:
...
func (o *ResearchOrchestrator) researchPlannerToolContracts(ctx context.Context, run ResearchRun) ([]researchPlannerToolContract, error) {
	...
	definitions := []researchPlannerToolContract{
		...
		{Name: ResearchWorkerToolSearchChatlog, Source: ResearchSourceChatlog, Arguments: map[string]string{
			"talker_ref": "required conversation or group reference, never a sender display name; use * only for a bounded all-conversation search with a non-empty keyword",
			"time_from":  "required RFC3339", "time_to": "required RFC3339",
			"sender_ref": "optional message-sender display-name hint; put the author here, not in talker_ref",
			"keyword":    "one literal message-content term; never concatenate independent terms with spaces; required when talker_ref is *",
