...
case "$all_args" in
  *./cmd/source-agent-updater* | *./cmd/wcplus-agent*)
    cat >"$output" <<'BUILT'
#!/bin/bash
printf '%s\n' '{"worker_type":"wcplus-worker","version":"0.2.0","protocol_version":"2026-08-01","platform":"darwin","architecture":"arm64","revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}'
BUILT
    ;;
  *) exit 64 ;;
esac
...
printf '%s\n' "$*" >"${UPDATER_CAPTURE:?}"
case "$*" in
  "--hold-lifecycle-lock --worker-type wcplus-worker")
    ...
    exit 1 ;;
  "--check --worker-type wcplus-worker" | "--check-uninstall --worker-type wcplus-worker") ;;
  "--install-config --worker-type wcplus-worker")
...
if grep -Fq 'life.executor.kbase.wcplus-agent.updater' "$plist_fixture" || grep -Fq -- '--run-pending' "$plist_fixture"; then
  echo "WC Plus worker plist contains updater job configuration" >&2
  exit 1
fi

updater_plist_fixture="$tmp_dir/wcplus-agent-updater.plist"
printf '%s\n' "$token_sentinel" | env -i PATH="$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
  KBASE_REMOTE_URL="https://kbase.example.invalid" \
  KBASE_SOURCE_AGENT_ID="wcplus-agent-1" \
  WCPLUSPRO_BASE_URL="http://127.0.0.1:5001" \
...
if [[ "$(/usr/libexec/PlistBuddy -c 'Print :Label' "$updater_plist_fixture")" != "life.executor.kbase.wcplus-agent.updater" ||
  ...
  "$(/usr/libexec/PlistBuddy -c 'Print :ProgramArguments:3' "$updater_plist_fixture")" != "wcplus-worker" ||
  ...
  echo "WC Plus updater plist does not have the fixed independent job contract" >&2
  exit 1
fi
if /usr/libexec/PlistBuddy -c 'Print :ProgramArguments:4' "$updater_plist_fixture" >/dev/null 2>&1; then
  echo "WC Plus updater plist contains extra program arguments" >&2
  exit 1
fi
if grep -Fq 'KBASE_SOURCE_AGENT_TOKEN' "$updater_plist_fixture" ||
  ...
  echo "WC Plus updater plist contains forbidden secret, environment, or process-group configuration" >&2
  exit 1
fi
...
case "$target" in
  *wcplus-agent.updater*) state_file="${UPDATER_LAUNCHCTL_STATE:-${LAUNCHCTL_STATE:?}}" ;;
...
installed_updater_plist="$tmp_dir/home/LaunchAgents/life.executor.kbase.wcplus-agent.updater.plist"
...
for failure_boundary in keychain validation plist bootout bootstrap kickstart final-print; do
  ...
  printf '%s\n' "$token_input" | env -i PATH="$tmp_dir/build-bin:$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
    ...
    WCPLUS_AGENT_LOG_DIR="$tmp_dir/install logs" \
    WCPLUS_AGENT_PLIST_PATH="$installed_plist" \
    WCPLUS_AGENT_UPDATER_PLIST_PATH="$installed_updater_plist" \
    KEYCHAIN_DIR="$tmp_dir/home/keychain" \
  ...
  if [[ "$failure_boundary" == keychain ]]; then
    if [[ -e "$tmp_dir/home/keychain/main" || -e "$tmp_dir/home/keychain/backup" ]]; then
      echo "WC Plus keychain fault did not restore prior absence" >&2
      exit 1
    fi
  ...
  if [[ "$(<"$tmp_dir/service-state")" != loaded || "$(<"$tmp_dir/updater-service-state")" != loaded ]]; then
    echo "WC Plus $failure_boundary fault did not restart both old services" >&2
    exit 1
  fi
...
env -i PATH="$PATH" HOME="$tmp_dir/home" \
  WCPLUS_AGENT_INSTALL_DIR="$tmp_dir/home/install" \
  WCPLUS_AGENT_PLIST_PATH="$tmp_dir/home/LaunchAgents/life.executor.kbase.wcplus-agent.plist" \
