...
usage() {
  echo "usage: uninstall-chatlog-agent-macos.sh [--check] [--delete-state] [--delete-logs]" >&2
}
while [[ $# -gt 0 ]]; do
  case "$1" in
    --check) mode=check ;;
...
if [[ "$(uname -s)" != Darwin ]]; then
  echo "chatlog-agent uninstallation requires macOS" >&2
  exit 1
fi
if [[ "$delete_state" == true && -z "$state_dir" ]]; then
  echo "CHATLOG_AGENT_STATE_DIR is required with --delete-state" >&2
  exit 2
fi
...
assert_safe_private_path() {
  ...
  [[ "$candidate" == "$home"/* && "$candidate" != "$home" && "$candidate" != "$home/" ]] || {
    echo "refusing unsafe Chatlog path" >&2
    exit 2
  }
}
...
if [[ "$mode" == check ]]; then
  echo "chatlog-agent uninstall configuration is valid"
  exit 0
fi
...
if [[ ! -d "$install_dir" ]]; then
  if [[ -e "$plist_path" || -e "$updater_plist_path" ]]; then
    echo "Chatlog install directory is missing while LaunchAgent state remains" >&2
    exit 1
  fi
else
  ...
  worker="$install_dir/chatlog-agent"
  ...
  if [[ -x "$updater" && -d "$install_dir/.source-agent-staging" && -d "$install_dir/.source-agent-handoff" ]]; then
    "$updater" --check-uninstall --worker-type chatlog-worker >/dev/null 2>&1 || {
      echo "Chatlog has unresolved update state; repair it before uninstall" >&2
      exit 1
    }
  fi
  managed_worker_uninstall_run "$home" chatlog-agent "$worker" "$updater" \
    "$plist_path" "$updater_plist_path" "$config" "gui/$(id -u)" "$label" "$updater_label" chatlog-worker
fi
...
echo "chatlog-agent uninstalled; shared transport token preserved"
