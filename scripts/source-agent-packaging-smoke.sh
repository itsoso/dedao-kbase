...
if ! grep -Fq 'mktemp "${TMPDIR:-/tmp}/source-agent.plist.XXXXXX"' "$install_script"; then
  echo "source-agent plist mktemp template is not suffix-unique" >&2
  exit 1
fi
first_plist_tmp="$(mktemp "$tmp_dir/source-agent.plist.XXXXXX")"
second_plist_tmp="$(mktemp "$tmp_dir/source-agent.plist.XXXXXX")"
if [[ "$first_plist_tmp" == "$second_plist_tmp" ]]; then
  echo "source-agent consecutive plist temporary names collided" >&2
  exit 1
fi
...
case "$*" in check-config | doctor) ;; *) exit 64 ;; esac
...
HOSTILE_STARTUP
chmod 0755 "$tmp_dir/probe-bin/dirname" "$tmp_dir/probe-bin/grep" "$tmp_dir/bin/source-agent" "$tmp_dir/bin/source-agent-updater"
...
if [[ $function_shadow_status -ne 0 || -e "$function_shadow_marker" ]]; then
  echo "source-agent installer executed an exported command-shadowing function" >&2
  exit 1
fi
...
if [[ $env_token_status -eq 0 ]]; then
  echo "source-agent installer accepted KBASE_SOURCE_AGENT_TOKEN from the environment" >&2
  exit 1
fi
if grep -Fq "$token_sentinel" "$tmp_dir/env-token.stdout" "$tmp_dir/env-token.stderr"; then
  echo "source-agent installer leaked the rejected environment token" >&2
  exit 1
fi
...
if [[ -e "$tmp_dir/first-child" || -e "$tmp_dir/hostile-startup-called" || -e "$tmp_dir/grep-called" ]]; then
  echo "source-agent installer used hostile PATH/startup code or sent its token to grep" >&2
  exit 1
fi
if grep -Fq "$token_sentinel" "$tmp_dir/render.stderr"; then
  echo "source-agent installer leaked its stdin token through xtrace" >&2
  exit 1
fi
...
bounded_fifo="$tmp_dir/source-bounded-token.fifo"
bounded_status_file="$tmp_dir/source-bounded-token.status"
bounded_sent_file="$tmp_dir/source-bounded-token.sent"
mkfifo "$bounded_fifo"
(
  set +e
  env -i PATH="$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
...
if [[ ! -e "$bounded_sent_file" || ! -e "$bounded_status_file" ]]; then
  ...
  echo "source-agent installer did not cap transport-token input consumption" >&2
  exit 1
fi
...
if [[ "$bounded_status" == 0 ]]; then
  echo "source-agent installer accepted a bounded oversize transport token" >&2
  exit 1
fi
...
sed \
  ...
  "$install_script" >"$tmp_dir/install-fixture/install-source-agent-macos.sh"
...
chmod 0755 "$tmp_dir/install-fixture/install-source-agent-macos.sh"
transaction_install_script="$tmp_dir/install-fixture/install-source-agent-macos.sh"
install_dir="$tmp_dir/home/install with spaces"
installed_plist="$tmp_dir/home/LaunchAgents/life.executor.kbase.source-agent.plist"
...
if [[ $installed_updater_status -eq 0 || "$(<"$installed_worker")" != "old-worker" || "$(<"$installed_updater")" != "old-updater" ]]; then
  echo "installed-updater failure did not restore the old artifact pair" >&2
  exit 1
fi
if [[ "$(<"$installed_plist")" != "old-plist" || -e "$tmp_dir/launchctl-called" ]]; then
  echo "installed-updater failure changed plist or launch state" >&2
  exit 1
fi
if ! grep -Fq 'managed_worker_install_begin' "$install_script"; then
  echo "source-agent installer has no full install transaction before installed updater gate" >&2
  exit 1
fi
...
for failure_boundary in keychain validation plist bootout bootstrap kickstart final-print; do
  ...
  if grep -Fq 'new-fixed-token' "$tmp_dir/matrix-$failure_boundary.stdout" "$tmp_dir/matrix-$failure_boundary.stderr" ||
    ...
    echo "source-agent $failure_boundary fault leaked Keychain material" >&2
    exit 1
  fi
done
