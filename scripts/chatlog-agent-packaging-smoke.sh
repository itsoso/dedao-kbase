...
build_script="$script_dir/build-chatlog-agent-macos.sh"
install_script="$script_dir/install-chatlog-agent-macos.sh"
uninstall_script="$script_dir/uninstall-chatlog-agent-macos.sh"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/chatlog-agent-packaging.XXXXXX")"
...
if ! grep -Fq 'mktemp "${TMPDIR:-/tmp}/chatlog-agent.plist.XXXXXX"' "$install_script"; then
  echo "Chatlog plist mktemp template is not suffix-unique" >&2
  exit 1
fi
first_plist_tmp="$(mktemp "$tmp_dir/chatlog-agent.plist.XXXXXX")"
second_plist_tmp="$(mktemp "$tmp_dir/chatlog-agent.plist.XXXXXX")"
if [[ "$first_plist_tmp" == "$second_plist_tmp" ]]; then
  echo "Chatlog consecutive plist temporary names collided" >&2
  exit 1
fi
...
case "$all_args" in
  *./cmd/source-agent-updater* | *./cmd/chatlog-agent*)
    cat >"$output" <<'BUILT'
#!/bin/bash
printf '%s\n' '{"worker_type":"chatlog-worker","version":"0.2.0","protocol_version":"2026-08-01","platform":"darwin","architecture":"arm64","revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}'
BUILT
    ;;
  *) exit 64 ;;
esac
...
run_fixture_build() {
  env PATH="$tmp_dir/build-bin:$PATH" \
    CHATLOG_AGENT_BINARY_PATH="$1" \
    CHATLOG_AGENT_UPDATER_BINARY_PATH="$2" \
...
mkdir -p "$publish_dir"
worker_output="$publish_dir/chatlog-agent"
...
different_worker="$tmp_dir/worker-parent/chatlog-agent"
...
exit 92
PROBE_GREP
cat >"$tmp_dir/bin/chatlog-agent" <<'WORKER'
#!/bin/bash
set -euo pipefail
...
if [[ "$*" == "build-info" ]]; then
  printf '%s\n' '{"worker_type":"chatlog-worker","version":"0.1.0","protocol_version":"2026-08-01","platform":"darwin","architecture":"arm64","revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}'
  exit 0
fi
[[ "$*" == "check-config" || "$*" == "doctor" ]]
...
case "${CHATLOG_BASE_URL:-http://127.0.0.1:5030}" in
  *\?* | *\#* | *://*@*) exit 66 ;;
...
if [[ "$*" == doctor ]]; then
  [[ "${FAIL_INSTALLED_DOCTOR:-false}" != true ]] || exit 97
  printf '%s\n' '{"ok":true,"chatlog_read":true,"remote_auth":true}'
fi
WORKER
...
printf '%s\n' "$*" >"${UPDATER_CAPTURE:?}"
case "$*" in
  "--build-info --worker-type chatlog-worker")
    printf '%s\n' '{"worker_type":"chatlog-worker","version":"0.2.0","protocol_version":"2026-08-01","platform":"darwin","architecture":"arm64","revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}'
    exit 0
    ;;
  "--hold-lifecycle-lock --worker-type chatlog-worker")
    ...
    exit 1 ;;
  "--check --worker-type chatlog-worker" | "--check-uninstall --worker-type chatlog-worker") ;;
  "--install-config --worker-type chatlog-worker")
...
HOSTILE_STARTUP
chmod 0755 "$tmp_dir/probe-bin/dirname" "$tmp_dir/probe-bin/grep" "$tmp_dir/bin/chatlog-agent" "$tmp_dir/bin/source-agent-updater"
...
printf '%s\n' 'function-shadow-token' | env -i PATH="$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
  ...
  KBASE_REMOTE_URL="https://kbase.example.invalid" \
  KBASE_SOURCE_AGENT_ID="chatlog-agent-1" \
  CHATLOG_AGENT_STATE_DIR="$tmp_dir/state" \
  CHATLOG_AGENT_BINARY_PATH="$tmp_dir/bin/chatlog-agent" \
  CHATLOG_AGENT_UPDATER_BINARY_PATH="$tmp_dir/bin/source-agent-updater" \
...
if [[ $function_shadow_status -ne 0 || -e "$function_shadow_marker" ]]; then
  echo "Chatlog installer executed an exported command-shadowing function" >&2
  exit 1
fi
...
for name in KBASE_REMOTE_URL KBASE_SOURCE_AGENT_ID CHATLOG_AGENT_STATE_DIR; do
...
if [[ $env_token_status -eq 0 ]]; then
  echo "Chatlog installer accepted KBASE_SOURCE_AGENT_TOKEN from the environment" >&2
  exit 1
fi
if grep -Fq "$token_sentinel" "$tmp_dir/env-token.stdout" "$tmp_dir/env-token.stderr"; then
  echo "Chatlog installer leaked the rejected environment token" >&2
  exit 1
fi

plist_fixture="$tmp_dir/chatlog-agent.plist"
printf '%s\n' "$token_sentinel" | env -i PATH="$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
  ...
  CHATLOG_AGENT_STATE_DIR="$tmp_dir/state" \
  CHATLOG_AGENT_LOG_DIR="$tmp_dir/logs" \
  CHATLOG_AGENT_BINARY_PATH="$tmp_dir/bin/chatlog-agent" \
  CHATLOG_AGENT_UPDATER_BINARY_PATH="$tmp_dir/bin/source-agent-updater" \
...
if [[ -e "$tmp_dir/first-child" || -e "$tmp_dir/hostile-startup-called" || -e "$tmp_dir/grep-called" ]]; then
  echo "Chatlog installer used hostile PATH/startup code or sent its token to grep" >&2
  exit 1
fi
if grep -Fq "$token_sentinel" "$tmp_dir/render.stderr"; then
  echo "Chatlog installer leaked its stdin token through xtrace" >&2
  exit 1
fi
if grep -Fq 'KBASE_SOURCE_AGENT_TOKEN' "$plist_fixture" || grep -Fq "$token_sentinel" "$plist_fixture"; then
  echo "Chatlog LaunchAgent plist contains the transport token" >&2
  exit 1
fi
grep -Fq 'life.executor.kbase.chatlog-agent' "$plist_fixture"
if grep -Fq 'life.executor.kbase.source-agent</string>' "$plist_fixture"; then
  echo "Chatlog plist uses the WeChat worker identity" >&2
  exit 1
fi
grep -Fq "$tmp_dir/bin/chatlog-agent" "$plist_fixture"
grep -Fq "$tmp_dir/state" "$plist_fixture"
grep -Fq "$tmp_dir/logs" "$plist_fixture"
grep -Fxq -- '--check --worker-type chatlog-worker' "$tmp_dir/updater-args"
grep -Fxq -- 'check-config' "$tmp_dir/worker-args"
if grep -Fq 'life.executor.kbase.chatlog-agent.updater' "$plist_fixture" || grep -Fq -- '--run-pending' "$plist_fixture"; then
  echo "Chatlog worker plist contains updater job configuration" >&2
  exit 1
fi

updater_plist_fixture="$tmp_dir/chatlog-agent-updater.plist"
printf '%s\n' "$token_sentinel" | env -i PATH="$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
  KBASE_REMOTE_URL="https://kbase.example.invalid" \
  KBASE_SOURCE_AGENT_ID="chatlog-agent-1" \
  CHATLOG_BASE_URL="http://127.0.0.1:5030" \
...
if [[ "$(/usr/libexec/PlistBuddy -c 'Print :Label' "$updater_plist_fixture")" != "life.executor.kbase.chatlog-agent.updater" ||
  ...
  "$(/usr/libexec/PlistBuddy -c 'Print :ProgramArguments:3' "$updater_plist_fixture")" != "chatlog-worker" ||
  ...
  echo "Chatlog updater plist does not have the fixed independent job contract" >&2
  exit 1
fi
if /usr/libexec/PlistBuddy -c 'Print :ProgramArguments:4' "$updater_plist_fixture" >/dev/null 2>&1; then
  echo "Chatlog updater plist contains extra program arguments" >&2
  exit 1
fi
if grep -Fq 'KBASE_SOURCE_AGENT_TOKEN' "$updater_plist_fixture" ||
  ...
  echo "Chatlog updater plist contains forbidden secret, environment, or process-group configuration" >&2
  exit 1
fi
...
for invalid_remote in \
  ...
  if [[ $invalid_status -eq 0 ]]; then
    echo "Chatlog installer accepted invalid remote URL" >&2
    exit 1
  fi
done
...
printf '%s\n' "$token_sentinel" | env -i PATH="$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
  ...
  KBASE_SOURCE_AGENT_ID="chatlog-agent-1" \
  CHATLOG_BASE_URL="http://127.0.0.1:5030/?debug=1" \
  CHATLOG_AGENT_STATE_DIR="$tmp_dir/state" \
  CHATLOG_AGENT_BINARY_PATH="$tmp_dir/bin/chatlog-agent" \
  CHATLOG_AGENT_UPDATER_BINARY_PATH="$tmp_dir/bin/source-agent-updater" \
  PROBE_CAPTURE="$tmp_dir/invalid-chatlog-first-child" \
  WORKER_CAPTURE="$tmp_dir/invalid-chatlog-worker-args" \
  UPDATER_CAPTURE="$tmp_dir/invalid-chatlog-updater-args" \
  GREP_CALLED_MARKER="$tmp_dir/invalid-chatlog-grep-called" \
  "$install_script" --check >/dev/null 2>&1
invalid_chatlog_status=$?
set -e
if [[ $invalid_chatlog_status -eq 0 ]]; then
  echo "Chatlog installer accepted invalid local URL" >&2
  exit 1
fi

bounded_fifo="$tmp_dir/chatlog-bounded-token.fifo"
bounded_status_file="$tmp_dir/chatlog-bounded-token.status"
bounded_sent_file="$tmp_dir/chatlog-bounded-token.sent"
mkfifo "$bounded_fifo"
(
  set +e
  env -i PATH="$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
...
if [[ ! -e "$bounded_sent_file" || ! -e "$bounded_status_file" ]]; then
  ...
  echo "Chatlog installer did not cap transport-token input consumption" >&2
  exit 1
fi
...
if [[ "$bounded_status" == 0 ]]; then
  echo "Chatlog installer accepted a bounded oversize transport token" >&2
  exit 1
fi
...
if [[ $oversize_status -eq 0 ]]; then
  echo "Chatlog installer accepted an oversize transport token" >&2
  exit 1
fi
...
case "$target" in
  *chatlog-agent.updater*) state_file="${UPDATER_LAUNCHCTL_STATE:-${LAUNCHCTL_STATE:?}}" ;;
...
sed \
  ...
  "$install_script" >"$tmp_dir/install-fixture/install-chatlog-agent-macos.sh"
...
chmod 0755 "$tmp_dir/install-fixture/install-chatlog-agent-macos.sh"
transaction_install_script="$tmp_dir/install-fixture/install-chatlog-agent-macos.sh"
install_dir="$tmp_dir/home/install with spaces"
installed_plist="$tmp_dir/home/LaunchAgents/life.executor.kbase.chatlog-agent.plist"
mkdir -p "$install_dir" "$(dirname "$installed_plist")"
canonical_install_dir="$(cd "$install_dir" && pwd -P)"
installed_worker="$canonical_install_dir/chatlog-agent"
installed_updater="$canonical_install_dir/source-agent-updater"
installed_updater_plist="$tmp_dir/home/LaunchAgents/life.executor.kbase.chatlog-agent.updater.plist"
...
printf '%s\n' "$token_sentinel" | env -i PATH="$tmp_dir/build-bin:$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
  ...
  CHATLOG_AGENT_BINARY_PATH="$tmp_dir/bin/chatlog-agent" \
  CHATLOG_AGENT_UPDATER_BINARY_PATH="$tmp_dir/bin/source-agent-updater" \
  CHATLOG_AGENT_INSTALL_DIR="$install_dir" \
  CHATLOG_AGENT_STATE_DIR="$tmp_dir/install state" \
  CHATLOG_AGENT_LOG_DIR="$tmp_dir/install logs" \
  CHATLOG_AGENT_PLIST_PATH="$installed_plist" \
...
if [[ $installed_updater_status -eq 0 || "$(<"$installed_worker")" != "old-worker" || "$(<"$installed_updater")" != "old-updater" ]]; then
  echo "installed Chatlog updater failure did not restore the old artifact pair" >&2
  exit 1
fi
if [[ "$(<"$installed_plist")" != "old-plist" || -e "$tmp_dir/launchctl-called" ]]; then
  echo "installed Chatlog updater failure changed plist or launch state" >&2
  exit 1
fi
if ! grep -Fq 'managed_worker_install_begin' "$install_script"; then
  echo "Chatlog installer has no full install transaction before installed updater gate" >&2
  exit 1
fi

for failure_boundary in keychain validation plist bootout bootstrap kickstart final-print doctor; do
  printf 'old-worker' >"$installed_worker"
  printf 'old-updater' >"$installed_updater"
  printf 'old-plist' >"$installed_plist"
  ...
  printf '%s\n' "$token_input" | env -i PATH="$tmp_dir/build-bin:$tmp_dir/probe-bin:$PATH" HOME="$tmp_dir/home" \
    ...
    CHATLOG_AGENT_LOG_DIR="$tmp_dir/install logs" \
    CHATLOG_AGENT_PLIST_PATH="$installed_plist" \
    CHATLOG_AGENT_UPDATER_PLIST_PATH="$installed_updater_plist" \
    KEYCHAIN_DIR="$tmp_dir/home/keychain" \
    ...
    FAIL_INSTALLED_DOCTOR="$fail_doctor" \
  ...
  if [[ $matrix_status -eq 0 ]]; then
    echo "Chatlog $failure_boundary fault unexpectedly succeeded" >&2
    exit 1
  fi
  if [[ "$(<"$installed_worker")" != old-worker || "$(<"$installed_updater")" != old-updater ||
    ...
    echo "Chatlog $failure_boundary fault did not restore old files" >&2
    exit 1
  fi
  if [[ "$failure_boundary" == keychain ]]; then
    if [[ -e "$tmp_dir/home/keychain/main" || -e "$tmp_dir/home/keychain/backup" ]]; then
      echo "Chatlog keychain fault did not restore prior absence" >&2
      exit 1
    fi
  elif [[ "$(<"$tmp_dir/home/keychain/main")" != old-fixed-token || -e "$tmp_dir/home/keychain/backup" ]]; then
    echo "Chatlog $failure_boundary fault did not restore the old Keychain account" >&2
    exit 1
  fi
  if [[ "$(<"$tmp_dir/service-state")" != loaded || "$(<"$tmp_dir/updater-service-state")" != loaded ]]; then
    echo "Chatlog $failure_boundary fault did not restart both old services" >&2
    exit 1
  fi
  if compgen -G "$tmp_dir/home/Library/Application Support/KBase/.managed-worker-install.lock-ready.*" >/dev/null ||
    ...
    echo "Chatlog $failure_boundary fault left transaction state" >&2
    exit 1
  fi
  if grep -Fq 'new-fixed-token' "$tmp_dir/matrix-$failure_boundary.stdout" "$tmp_dir/matrix-$failure_boundary.stderr" ||
    ...
    echo "Chatlog $failure_boundary fault leaked Keychain material" >&2
    exit 1
  fi
done
...
env -i PATH="$PATH" HOME="$tmp_dir/home" \
  CHATLOG_AGENT_INSTALL_DIR="$tmp_dir/home/install" \
  CHATLOG_AGENT_PLIST_PATH="$tmp_dir/home/LaunchAgents/life.executor.kbase.chatlog-agent.plist" \
  bash "$uninstall_script" --check >/dev/null

grep -Fq './cmd/chatlog-agent' "$build_script"
...
grep -Fq 'dedao-kbase/chatlog-agent' "$install_script"
if grep -Fq 'dedao-kbase/bin}' "$install_script"; then
  echo "Chatlog worker must not share the WC Plus updater configuration directory" >&2
  exit 1
fi
...
grep -Fq 'source-agent-updater" --check --worker-type chatlog-worker' "$install_script"
grep -Fq '"$installed_worker" doctor' "$install_script"
...
echo "chatlog agent packaging smoke passed"
