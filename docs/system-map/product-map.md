...
Downloaded ebook HTML
  ...
  -> exports for health KB, Health Authority Pack, quant rule cards, and NotebookLM bridge packages
```

The desktop workbench is Wails-native and calls generated `frontend/wailsjs/go/backend/App.*` bindings. The private kbase server is HTTP-native and exposes Bearer-protected book, search, System KB, Health Authority Pack, prompt, chat, chat-history, Dedao session/login, read-only course browser/detail/article reading, read-only ebook bookshelf/detail/chapter-page reading, listening-book bookshelf/detail/transcript reading, and job endpoints under `/api/*`.

## Web KBase Expansion Point

The web UI does not reuse the Wails runtime. It is an independent browser app served by `cmd/kbase-server`, using the existing Bearer-protected HTTP API. This keeps desktop and browser runtimes separate while sharing the same `BookKnowledgeStore`, prompt generator, TokenPlan chat layer, chat history store, job store, System KB export files, project collections, Health Authority Pack builder, and server-side Dedao login config. TokenPlan secrets, Dedao cookies, and ebook read tokens remain server-side in environment/configuration. Chat history uses SQLite when cgo is available and a JSONL file fallback for cross-compiled `CGO_ENABLED=0` server builds. Online jobs are recorded in `jobs.json` so Linux cross-compiled deployments can create NotebookLM, health KB, quant rule-card, ebook download, ebook-to-kbase sync, listening-book download, and listening-book-to-kbase sync tasks without SQLite. Browser navigation exposes the current online surfaces: library search, book study, QR login, personal center status, read-only course browser/detail/article reading, ebook bookshelf/detail/chapter reading with download and kbase sync actions, listening-book bookshelf/detail/transcript reading with audio/PDF/Markdown downloads, kbase sync actions, and TokenPlan analysis, jobs, System KB, Health Authority Pack, Skills/API discovery, and basic ops status.

## Health Authority Pack Boundary

`health_authority_pack_v1` is derived from the existing health project verification collection. It preserves source refs (`book_id`, `chapter_id`, `claim_id`, citations, `source_hash`) and caps Dedao-only claims to education/context use. Downstream health-llm-driven imports must dry-run and review before serving; records may not become diagnosis, treatment, medication, dosage, or emergency action support directly.
